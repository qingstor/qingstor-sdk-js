...
module.exports = function () {
    ...
    this.Then(/^the bucket is initialized$/, function (callback) {
        callback(null, callback(null, test_bucket.should.not.to.be.undefined));
    });
    ...
    this.When(/^put same bucket again$/, function (callback) {
        test_bucket.put(function (err, res, data) {
            test_res = res;
            callback();
        });
    });
    ...
    this.Then(/^list objects keys count is (\d+)$/, function (arg1, callback) {
        callback(null, JSON.parse(test_data)["keys"].length.toString().should.least(arg1));
    });
    this.When(/^head bucket$/, function (callback) {
        test_bucket.head(function (err, res, data) {
            test_res = res;
            test_data = data;
            console.log(data);
            callback();
    ...
    this.When(/^get bucket statistics$/, function (callback) {
        test_bucket.getStatistics(function (err, res, data) {
            test_res = res;
            test_data = data;
            console.log(data);
            callback();
    ...
    this.Then(/^get bucket statistics status is "([^"]*)"$/, function (arg1, callback) {
        callback(null, JSON.parse(test_data)['status'].toString().should.eql(arg1));
    });
};
